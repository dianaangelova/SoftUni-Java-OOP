...
public abstract class BaseHouse implements House {
    ...
    public int sumSoftness() {
        return toys.stream().mapToInt(t -> t.getSoftness()).sum();
    }
    ...
    public String getStatistics() {
        ...
        String catsOutput = cats.isEmpty() ? "none" : cats.stream()
                .map(c -> c.getName())
                .collect(Collectors.joining(" "));

        return String.format("%s %s:%n" +
                "Cats: %s%n" +
                "Toys: %d Softness: %d", name, this.getClass().getSimpleName(), catsOutput, toys.size(), sumSoftness());
    }
